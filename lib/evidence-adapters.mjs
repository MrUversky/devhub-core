...
export function validateEvidenceBinding(binding, adapter) {
  ...
  exactFields(binding, new Set([
    ...
    "credentialEnv", "checks", "freshForSeconds",
  ]), "binding");
...
function normalizeDeployment(deployment) {
  ...
  exactFields(deployment, new Set(["identity", "revision", "url", "host"]), "observation.deployment");
  const result = {};
...
function normalizeSuccess(binding, observation, now) {
  ...
  if (observation.evidence.length > MAX_EVIDENCE_ITEMS) {
    fail("invalid-adapter-result", `observation.evidence must contain at most ${MAX_EVIDENCE_ITEMS} items`);
  }
...
export async function runEvidenceAdapter({ binding, adapter, environment = {}, now, cache = null }) {
  ...
  let observation;
  try {
    observation = await adapter.collect(immutable({
