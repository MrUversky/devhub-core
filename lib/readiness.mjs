...
const INHERITABLE_READINESS_FIELDS = ["profile", "owner", "dataClassification", "costModel"];

export function resolveServiceReadinessContext(project, service) {
  const defaults = project?.readinessDefaults ?? {};
  const explicit = service?.readiness ?? null;
  const fields = {};
  const readiness = {};

  for (const field of INHERITABLE_READINESS_FIELDS) {
    if (explicit?.[field] !== undefined) {
      fields[field] = { value: explicit[field], provenance: "service" };
      readiness[field] = explicit[field];
    } else if (defaults[field] !== undefined) {
      fields[field] = { value: defaults[field], provenance: "project" };
      readiness[field] = defaults[field];
    } else {
      fields[field] = { value: null, provenance: "absent" };
    }
  }

  for (const field of ["deployment", "dependencies"]) {
    if (explicit?.[field] !== undefined) readiness[field] = explicit[field];
  }
  readiness.evidence = explicit?.evidence ?? [];

  const hasContext = explicit !== null || Object.values(fields).some((field) => field.provenance !== "absent");
  return {
    readiness: hasContext ? readiness : null,
    fields,
    evidenceProvenance: explicit ? "service" : "absent",
  };
}
