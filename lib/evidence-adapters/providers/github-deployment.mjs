...
export function createGitHubDeploymentAdapter({ fetch: fetchImpl = globalThis.fetch, timeoutMs, maxResponseBytes } = {}) {
  ...
  return Object.freeze({
    ...
    async collect(request) {
      ...
      if ((request.limits?.maxPages ?? 4) < 4) return unavailableResult("provider-page-limit-exceeded");
      ...
      const [workflow, run, deployment, status] = await Promise.all([
        ...
        githubJson(fetchImpl, `${prefix}/actions/runs/${identity.runId}`, request.credential, requestOptions),
        githubJson(fetchImpl, `${prefix}/deployments/${identity.deploymentId}`, request.credential, requestOptions),
        githubJson(fetchImpl, `${prefix}/deployments/${identity.deploymentId}/statuses/${identity.statusId}`, request.credential, requestOptions),
      ]);
