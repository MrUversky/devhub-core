...
- Public Windows CI now disables Git archive EOL conversion before extracting
  and verifying the reviewed manifest, preserving canonical released bytes
  without normalizing or weakening the manifest contract.

## 1.0.0-rc.3 — 2026-08-21

### Fixed

- Public-manifest verification on Windows now runs against canonical
  `git archive` bytes, excludes only root Git transport metadata, and keeps
  every released path and line ending byte-bound.
